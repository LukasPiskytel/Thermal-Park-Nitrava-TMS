...
function formatAxisTime(msValue) {
  return new Date(msValue).toLocaleString('sk-SK', {
    day: '2-digit',
    month: '2-digit',
    hour: '2-digit',
    minute: '2-digit',
...
function buildChart(fetchLog) {
  ...
  const width = 100;
  const height = 100;
  const margin = { top: 8, right: 6, bottom: 16, left: 16 };
  ...
  const circles = fetchLog
    .map((entry) => {
      ...
      return `<circle class="point" cx="${cx}" cy="${cy}" r="0.8" />`;
    })
    .join('');
  ...
  const yTickLabels = yTicks
    .map((tick) => {
      const y = toY(tick).toFixed(2);
      return `<text class="tick-label" x="${(margin.left - 1.2).toFixed(2)}" y="${y}" text-anchor="end" dominant-baseline="middle">${formatTemperature(tick)}</text>`;
    })
    .join('');

  const xTicks = [minTime, minTime + timeRange / 2, maxTime];
  const xTickLabels = xTicks
    .map((tick) => {
      const x = toX(tick).toFixed(2);
      const label = escapeHtml(formatAxisTime(tick));
      return `<text class="tick-label" x="${x}" y="${(height - 2).toFixed(2)}" text-anchor="middle">${label}</text>`;
    })
    .join('');
  ...
  return `
    <div class="chart-wrap">
      <svg class="chart" viewBox="0 0 ${width} ${height}" preserveAspectRatio="none">
        ...
        <text class="axis-label" x="3" y="${(height / 2).toFixed(2)}" transform="rotate(-90, 3, ${
          height / 2
        })">Teplota (°C)</text>
        <text class="axis-label" x="${(width / 2).toFixed(2)}" y="${(height - 7).toFixed(2)}" text-anchor="middle">Cas</text>
